...
public class SampleController {
    // 컨트롤러는 url 생성과 프론트를 연결하는 부분으로 과거의 servlet-context.xml과 같은 역할을 함.

    @GetMapping("/hello") //192.168.111.105:80/hello -> void -> hello.html
    ...
    public void ex2(Model model){
        ...
        List<String> strList = IntStream.range(1,10) // 1~10 까지 정수를 생성한다.
                .mapToObj(i -> "데이터"+i)
        ...
        model.addAttribute("list",strList);     // 64행에 만들어 진 객체 값
        model.addAttribute("map",map);          // 69행에 만들어 진 객체 값
        model.addAttribute("dto", sampleDTO);   //73행에 만들어진 객체 값

        // 리턴타입이 void임으로 /resources/templates/ex/ex2
    }

    @GetMapping("/ex/ex3") // http://192.168.111.105/ex/ex3 -> /resources/templates/ex/ex1.html
