...
public class BoardRepositoryTests {
    ...
    public void testInsert() {
        ...
        IntStream.rangeClosed(1, 100).forEach(i -> {
            // i 변수에 1~100 까지 의 100개의 정수를 반복해서 생성
    ...
    public void testSelect() {
        ...
        //                                       .findById(bno); slelct * from board where bno = bno;
    ...
    public void testUpdate() {
        ...
        Optional<Board> result = boardRepository.findById(bno); // bno를 찾아서 result 에 넣는다.

        Board board = result.orElseThrow(); // 가져온 값이 있으면 board 타입에 객체에 넣는다.

        board.change("수정테스트 제목", "수정테스트 내용"); // 제목과 내용만 수정할 수 있는 메서드

        boardRepository.save(board); // .save메서드는 pk값이 없으면 insert, pk값이 있으면 update 한다.
        // Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
    ...
    public void testPaging(){
        // .findALl() 메서드는 모든 리스트를 출력하는 메서드 select * from board;
        // 전체리스트에 페이징과 정렬 기법도 추가 해보자.

        Pageable pageable = PageRequest.of(0,10, Sort.by("bno").descending());
        //                                   시작번호,페이지당 데이터 갯수        번호를 기준으로 내림차순 정렬!!!
        ...
        // 콘솔에 결과를 출력해보자.
        List<Board> boardList = result.getContent(); // 페이징 처리 된 내용을 가져와라.

        boardList.forEach(board -> log.info(board));
        // forEach 는 인덱스를 사용하지 않고 앞에서 부터 객체를 리턴한다.
